...
unsigned int	SaveDataF0_FRQCR =FRQCR_default;		// PLL:x16 IFC:1/4 SFC:1/8 BFC:1/8 PFC:1/16
...
void SaveFile(){
	...
	state=MCSPutVar2((unsigned char*)"@PTUNE", (unsigned char*)"Ptune2", dataN*4, buf);
	switch ( state ) {
		case 0:
			break;
		case 37:									//  file already exists
			...
			MCSDelVar2((unsigned char*)"@PTUNE",(unsigned char*)"Ptune2");
			state=MCSPutVar2((unsigned char*)"@PTUNE", (unsigned char*)"Ptune2", dataN*4, buf);
			break;	
...
void LoadFile(int check){
	...
	state=MCSGetDlen2((unsigned char*)"@PTUNE", (unsigned char*)"Ptune2",&x);
	if (state!=0) {
