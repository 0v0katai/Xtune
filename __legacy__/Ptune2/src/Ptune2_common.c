...
void Bkey_SetFlag( short flagpattern ){	//
	__asm__(
	    ...
	    "mov.l bkey_SetFlag, r0\n"
	    "jmp @r2\n"
	    "nop\n"
	    "syscall_adress: .long 0x80020070\n"
	    "bkey_SetFlag: .long 0x0EA2");
}
void Bkey_ClrFlag( short flagpattern ){	//
	__asm__(
	    "mov.l syscall_adress1, r2\n"
	    "mov.l bkey_ClrFlag, r0\n"
	    "jmp @r2\n"
	    "nop\n"
	    "syscall_adress1: .long 0x80020070\n"
	    "bkey_ClrFlag: .long 0x0EA4");
}
...
void StatusArea_sub(){
	...
	StatusArea( "Ptune2 (PRIZM TuneUp Util) v1.24",TEXT_COLOR_RED);
}
