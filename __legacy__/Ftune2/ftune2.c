...
void about(){
	...
	PopUpWin(5);
	locate(3,2);Print((unsigned char *)"Ftune2 v1.40 ");
...
void update(int select,int select_func,int modify) {
	...
	if (select == -1){
		...
		locate(1,1); PrintRLine((unsigned char *)" Ftune2 for SH4A       ",21);
		if (SetupData & 128) {
					sprintf(buf,"%4.2fv",(float)MainBatteryPercentage()/100);
					PrintMini(17*6,2,(unsigned char *)buf, MINI_REV);
		}	else	PrintMini(17*6,2,(unsigned char *)"v1.40 ", MINI_REV);
